...
/* =========================================================
   KEEP CATEGORIES AND SUMMARY SCROLLING TOGETHER
========================================================= */

let syncingAdminHorizontalScroll = false;

function syncAdminHorizontalScroll(source, target){
    if(syncingAdminHorizontalScroll){
        return;
    }

    syncingAdminHorizontalScroll = true;
    target.scrollLeft = source.scrollLeft;

    window.requestAnimationFrame(function(){
        syncingAdminHorizontalScroll = false;
    });
}

adminCategoryScroll.addEventListener(
    'scroll',
    function(){
        syncAdminHorizontalScroll(
            adminCategoryScroll,
            adminSummaryWrap
        );
    },
...
adminSummaryWrap.addEventListener(
    'scroll',
    function(){
        syncAdminHorizontalScroll(
            adminSummaryWrap,
...
function renderPermanentAdminSummary(categoryId,summary){
    ...
    if(
        ...
        lines.textContent =
            `Events : ${formatAdminCount(summary.count)}`;
    }
        ...
        lines.innerHTML =
            `Sponsors : ${formatAdminCount(summary.count)}<br>` +
            `Amount : ${formatAdminMoney(summary.amount)}`;
    }
}
...
/* =========================================================
   RESOLVE REAL ADMIN NAME FOR CREATED BY
========================================================= */

const adminNameByMobile = new Map();

async function loadAdminNameMap(){

    adminNameByMobile.clear();
...
function getRealCreatedByName(record){
    ...
    const mobile =
        String(
            record?.created_by_mobile || ''
        ).trim();

    const mappedName =
        mobile
            ? adminNameByMobile.get(mobile)
            : '';
...
.on(
  ...
  () => {
    loadAllAdminSummaries();
    if(currentAdminCategory === "events"){
      loadAdminCategoryData("events");
    }
  }
)
...
.on(
  "postgres_changes",
  {
    event: "*",
    schema: "public",
    table: "moving_donations"
  },
  () => {
    loadAllAdminSummaries();
    if(currentAdminCategory === "sponsors"){
      loadAdminCategoryData("sponsors");
    }
  }
)
...
if(!document.getElementById('adminPinkShimmerStyle')){
    ...
    shimmerStyle.textContent = `
        ...
        #adminAccessContent > * {
            animation:adminContentFadeIn .22s ease both;
        }
        ...
        #adminCategoryScroll button:hover {
        ...
        #adminAccessContent table {
            background:transparent !important;
        }

        #adminAccessContent table tbody tr {
            background:transparent !important;
        }

        #adminAccessContent table tbody tr:hover {
            background:#f7f7f5 !important;
        }

        #adminAccessContent table td {
            background:transparent !important;
        }
        ...
        @media (max-width: 767px) {
            ...
            #adminSummaryWrap [data-summary-category] {
...
function renderAdminDataTable(
    ...
    records.forEach(function(record){
        ...
        config.columns.forEach(function(column){
            ...
            let displayValue =
                column.key === 'created_by_name'
                    ? getRealCreatedByName(record)
                    : record[column.key];
...
function renderAdminManagement(records){
    ...
    [nameInput,mobileInput,passwordInput].forEach(function(input){
    ...
    form.addEventListener('submit',async function(event){
        ...
        if(name.length < 2){
            status.textContent = 'Enter admin name';
            nameInput.focus();
            return;
        }
        ...
        try{
            ...
            /*
             * Do not reload the full Admin section here.
             * This avoids shimmer, blinking and scroll movement.
             * The newly saved admin will appear the next time
             * the Admin category is opened.
             */
            await loadAllAdminSummaries();
            ...
            status.textContent =
                error?.message || 'Unable to save admin';
    ...
    records.forEach(function(record,index){
        ...
        editButton.addEventListener('click',function(){
            ...
            editName.type = 'text';
            editName.maxLength = 60;
            editName.value = record.name || '';
            editName.placeholder = 'Admin name';

            const editMobile = document.createElement('input');
            editMobile.type = 'tel';
            editMobile.inputMode = 'numeric';
            editMobile.maxLength = 10;
            editMobile.value = record.mobile_number || '';
            editMobile.placeholder = '10-digit mobile number';

            const editPassword = document.createElement('input');
            editPassword.type = 'password';
            editPassword.placeholder =
                'New password (leave empty to keep old)';

            [editName,editMobile,editPassword].forEach(function(input){
                Object.assign(input.style,{
                    width:'100%',
                    height:'44px',
                    marginBottom:'9px',
            ...
            editMobile.addEventListener('input',function(){
                editMobile.value =
                    editMobile.value.replace(/\D/g,'').slice(0,10);
            });
            ...
            [cancelEdit,saveEdit].forEach(function(button){
            ...
            cancelEdit.style.background = '#eeeeee';
            cancelEdit.style.color = '#444';

            saveEdit.style.background = '#2e7d32';
            saveEdit.style.color = '#ffffff';

            const editStatus = document.createElement('div');

            Object.assign(editStatus.style,{
                minHeight:'18px',
                marginTop:'8px',
                color:'#d32f2f',
                fontFamily:'Poppins, sans-serif',
            ...
            saveEdit.addEventListener('click',async function(){
                ...
                /*
                 * Update the existing card directly.
                 * No full category reload, shimmer or page blink.
                 */
                record.name = updatedName;
                record.mobile_number = updatedMobile;
            ...
            editWrap.appendChild(editName);
            editWrap.appendChild(editMobile);
            editWrap.appendChild(editPassword);
...
/* =========================================================
   ...
   TABLE: moving_donations
   Add / Edit / +/- Amount / Delete — no alerts
========================================================= */

function renderSponsorManagement(records){
    ...
    addTitle.textContent = 'Add Sponsor';
    Object.assign(addTitle.style,{
        marginBottom:'10px',
        color:'#222',
        fontSize:'15px',
        fontWeight:'700'
    ...
    addName.type = 'text';
    addName.maxLength = 80;
    addName.placeholder = 'Sponsor name';
    ...
    addPrice.min = '0';
    addPrice.step = '1';
    addPrice.placeholder = 'Amount';

    [addName,addPrice].forEach(function(input){
        Object.assign(input.style,{
            width:'100%',
            height:'44px',
            padding:'0 12px',
    ...
    addButton.innerHTML = '<span style="font-size:20px;line-height:1;">+</span><span>Add</span>';
    ...
    function setSponsorStatus(element,text,isError){
        element.textContent = text || '';
        element.style.color = isError ? '#d32f2f' : '#5f5f5f';
    }

    addCard.addEventListener('submit',async function(event){
        event.preventDefault();

        const name = String(addName.value || '').trim().replace(/\s+/g,' ');
        const price = Number(addPrice.value);

        if(!name){
            setSponsorStatus(addStatus,'Enter sponsor name',true);
            addName.focus();
            return;
        }

        if(!Number.isFinite(price) || price < 0){
            setSponsorStatus(addStatus,'Enter valid amount',true);
            addPrice.focus();
            return;
        }
        ...
        setSponsorStatus(addStatus,'Saving...',false);

        const { error } = await adminSupabase
            .from('moving_donations')
            .insert({ name:name, price:price });

        if(error){
            console.error('Sponsor add failed:',error);
            setSponsorStatus(addStatus,error.message || 'Unable to add sponsor',true);
        ...
        addName.value = '';
        addPrice.value = '';
        setSponsorStatus(addStatus,'Added',false);
        addButton.disabled = false;
        addButton.innerHTML = '<span style="font-size:20px;line-height:1;">+</span><span>Add</span>';

        await loadAdminCategoryData('sponsors');
        await loadAllAdminSummaries();
    });

    addFields.appendChild(addName);
    addFields.appendChild(addPrice);
    addFields.appendChild(addButton);
    addCard.appendChild(addTitle);
    addCard.appendChild(addFields);
    addCard.appendChild(addStatus);
    ...
    if(safeRecords.length === 0){
        ...
        empty.textContent = 'No sponsors added';
    ...
    safeRecords.forEach(function(record){
        ...
        const row = document.createElement('div');
        row.dataset.adminSearch = `${record.name || ''} ${record.price || ''} ${record.id || ''}`;
        Object.assign(row.style,{
            width:'100%',
            padding:'10px',
            display:'grid',
            gridTemplateColumns:'minmax(0,1fr) minmax(110px,150px) auto auto auto auto',
            gap:'7px',
            alignItems:'center',
        ...
        priceInput.min = '0';
        priceInput.step = '1';
        priceInput.value = Number(record.price || 0);
        ...
        const minusButton = makeIconButton('<i class="fa-solid fa-minus"></i>','Decrease amount');
        const plusButton = makeIconButton('<i class="fa-solid fa-plus"></i>','Increase amount');
        const saveButton = makeIconButton('<i class="fa-solid fa-check"></i>','Save sponsor');
        const deleteButton = makeIconButton('<i class="fa-solid fa-trash"></i>','Delete sponsor');
        deleteButton.style.background = '#fff3f4';
        deleteButton.style.color = '#d32f2f';
        deleteButton.style.borderColor = '#ffe0e3';
        ...
        minusButton.addEventListener('click',function(){ adjustAmount(-1); });
        plusButton.addEventListener('click',function(){ adjustAmount(1); });
        ...
        saveButton.addEventListener('click',async function(){
            ...
            if(!name){
                status.textContent = 'Enter sponsor name';
                status.style.color = '#d32f2f';
                nameInput.focus();
                return;
            }
            ...
            saveButton.disabled = false;
            saveButton.innerHTML = '<i class="fa-solid fa-check"></i>';
            row.dataset.adminSearch = `${name} ${price} ${record.id || ''}`;
            await loadAllAdminSummaries();
        });
        ...
        deleteButton.addEventListener('click',async function(){
            ...
            setTimeout(function(){ row.remove(); },190);
            await loadAllAdminSummaries();
        });
    ...
    if(window.matchMedia && window.matchMedia('(max-width: 680px)').matches){
        addFields.style.gridTemplateColumns = '1fr 120px';
        addButton.style.gridColumn = '1 / -1';
        safeRecords.forEach(function(_,index){
            const row = list.children[index + (safeRecords.length ? 0 : 1)];
            if(row && row.dataset && row.dataset.adminSearch !== undefined){
                row.style.gridTemplateColumns = 'minmax(0,1fr) 120px repeat(4,40px)';
...
async function loadAdminCategoryData(categoryId){
    ...
    try{
        ...
        if(categoryId === 'images'){
            ...
            renderSponsorManagement(records);
        }
